...
| 2026-05-27 12:00 | wave-19-hitl-fire-and-forget | general-purpose | pre-wave-research | wave19-research | success: 7 authoritative citations confirming asyncio.create_task + strong-ref + add_done_callback is the canonical FastAPI fire-and-forget pattern for 2024-2025 (Python docs, FastAPI tutorial, Tiangolo discussion, Sentry AsyncioIntegration, SuperFastPython, Michael Kennedy, Leapcell); tradeoff = no durability across worker restarts (acceptable for HITL learning side-effect) | agent a18fed1d7c62a5ccc; pre-wave research per BRAIN/00_LAW_OF_ENTERPRISE_GRADE §0 |
| 2026-05-27 12:00 | wave-20-partial-index | general-purpose | pre-wave-research | wave20-research | success: 7 authoritative citations (Postgres docs Example 11.3, CREATE INDEX docs, Index-Only Scans, Heap blog, Use-The-Index-Luke, Supabase CLI Issue #2898, pganalyze) confirming `CREATE INDEX CONCURRENTLY ... WHERE bool_col = true` is the textbook pattern for rare-flag boolean indexes serving COUNT FILTER queries | agent aea3ae0411b6481ec; pre-wave research per BRAIN/00_LAW_OF_ENTERPRISE_GRADE §0 |
...
| 2026-05-27 | code-reviewer | wave-25 | PASS_WITH_NITS | bulk-verdict endpoint + toolbar ship; 1 HIGH (last-writer-wins on checkpoint_results JSON under concurrent reviewers — needs row lock or compare-and-swap) + 2 CRITICAL on learning fan-out backpressure (200 simultaneous LLM tasks) and tri-state chip dedupe race; selection origIndex uniqueness, optimistic patch safety, and card-body audio seek all verified clean |
