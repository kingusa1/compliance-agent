...
**As of 2026-05-23: Tip `4f3a905` on origin/main. Comprehensive dry-run battery (all green) + Railway Pro 24 GB perf audit, see [[04_Sessions/2026-05-23_Session_dry_runs_and_perf_audit]]. Pool tuned (pool_size 25, max_overflow 50, LIFO, query_cache_size 1200) for Pro plan headroom. Single biggest remaining win: migrate Railway service from us-west1 (San Francisco) to an EU region to match Supabase eu-west-1 — saves ~150ms per DB round-trip. Dry-run verdict: backend healthy, frontend 200 on every page, CI green on last 5 pushes, 48/48 touched tests pass.**

**Earlier 2026-05-18 (late PM): Tip `b7928e0` on `fix/westbury-supplier-prepass-agent-smell-chips-removed` (PR #2). 5-layer smart name detection fix shipped on top of the Westbury supplier-prepass branch. User's "name of agent and customer not appearing in transcript" verified as 4 distinct root causes: missing `_NAME_STOPWORDS` entries (Finding #3 "Is"), zero customer regex pre-pass, no Deal/Customer fallback when LLM gives up, no AAI-retry on PII redaction. All fixed + 14 new pytest cases. CI for `b7928e0`: vitest+gate GREEN, coverage+pytest IN_PROGRESS. See [[04_Sessions/2026-05-18_Session_smart_name_detection]]. Open follow-up: merge PR #2 → Railway auto-deploys → upload a new call to validate clean names.**

**Earlier 2026-05-18 (PM): Tip `d34ab12` on `fix/audit-2026-05-18-tracker-pii-aai-speakers` (PR #1). 5-hour autonomous Playwright sweep of every prod page + 1 real non-EON record lifecycle (Crosby Grange) caught 7 bugs; 5 backend + 1 frontend fix shipped on a feature branch. See [[04_Sessions/2026-05-18_Session_5h_autonomous_audit]]. Open follow-ups: merge PR #1 once both CI workflows green, Railway redeploys, then re-run the Playwright spot-checks listed at the bottom of that session log. Still recommended (carried from earlier sessions): rotate the OpenRouter + AssemblyAI keys.**

**Earlier 2026-05-18 (close-out): Tip `edfc746` on origin/main. CI BOTH workflows (coverage + test) are GREEN for the first time. AAI key set on Railway → two-layer transcript validation operational end-to-end. Queue Reviewed-tab badge now matches list. 25+ pre-existing pytest test-isolation bugs fixed at the conftest-level. See [[04_Sessions/2026-05-18_Session_aai_activation_queue_fix_ci_green]].**

**Earlier 2026-05-18 (morning): REPO IS PUBLIC. Tip `8bed1cb`. History rewritten via `git filter-repo` — leaked OpenRouter key purged from all 239 commits, README.md deleted from history, force-pushed. Coverage CI workflow GREEN. Two-layer transcript chips still rendering on prod. Pre-existing pytest test-workflow failures (test_claim_*, test_compliance_*) are documented tech debt that surfaced once Actions were unblocked. STILL pending user action: set `ASSEMBLYAI_API_KEY` on Railway + rotate the leaked OpenRouter key as defence-in-depth. See [[04_Sessions/2026-05-18_Session_public_repo_security_cleanup]].**

**Earlier 2026-05-18 (early morning): Two-layer Deepgram/AssemblyAI transcript validation + diarization fallback shipped to prod at tip `935e032`. Cross-validation module (`app/transcript_cross_validation.py`) + pipeline wire-up + chip on call detail + admin observability endpoints. PLUS browser-verified discovery: AAI is failing in prod with `ASSEMBLYAI_API_KEY not set` — set the env var on Railway and AAI starts producing transcripts. See [[04_Sessions/2026-05-17_Session_two_layer_transcript_validation]]. Edit-metadata hardening (Pydantic length caps + 422 shrink guard) bundled in same wave.**
