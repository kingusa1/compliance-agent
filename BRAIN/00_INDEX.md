...
**As of 2026-05-18 (close-out): Tip `edfc746` on origin/main. CI BOTH workflows (coverage + test) are GREEN for the first time this session. AAI key is set on Railway → two-layer transcript validation operational end-to-end. Queue Reviewed-tab badge now matches list (no more "badge says 1, list is empty"). 25+ pre-existing pytest test-isolation bugs fixed at the conftest-level. Still recommended: rotate the leaked OpenRouter key + the chat-supplied AssemblyAI key. See [[04_Sessions/2026-05-18_Session_aai_activation_queue_fix_ci_green]].**
