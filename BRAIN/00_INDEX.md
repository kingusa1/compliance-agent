...
**As of 2026-05-16 (early morning): POLLING ROLLBACK + SECOND-PASS DEAL MERGE + VULNERABILITY SCHEMA FIX.**
Read in order:

1. [[04_Sessions/2026-05-16_Session_polling_rollback_and_handoff]] — **MOST RECENT.** Reverted the aggressive `refetchInterval` polling from `eb5566d` because it was re-mounting `<audio>` and resetting playback to 0 every cycle. Real-time now = window-focus + reconnect refresh only; call detail polls ONLY while `status === processing`; queue keeps a slow 15s background refresh. Plus second-pass `_maybe_merge_into_existing_deal` invocation using `override_customer_name=business_name` (first pass had only the person name, so deal-linker never collapsed). Plus `risk_tag=None` on vulnerability flags (was `"Vulnerable"`, violated `ck_flags_risk_tag`, caused PendingRollbackError on every call's L2 step — same shape as the earlier `ck_call_segments_stage` bug). Plus spacebar guard on Override→Fail textarea. Plus Sonnet 4.6 routing via `cheap=True` on cheap detectors (supplier, names, business, call_type). Awais 4-call test: 4 calls → 3 deals (was 4); transcript drift too wide for fuzzy 0.80 to fully collapse. Tip backend + frontend both `e1c8d3b`.
